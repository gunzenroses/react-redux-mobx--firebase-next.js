...
const checkIsRoomSuitable = ({room, filters, key, subKey}: CheckIsRoomSuitable): boolean => {
  ...
  const isUnsuitableDateFilter =
    key === 'freeDays'
    && ((subKey === 'from' && new Date(room[key][subKey]) > new Date(filters[key][subKey]))
      || (subKey === 'to' && new Date(room[key][subKey]) < new Date(filters[key][subKey])));

  return isUnsuitableBooleanFilter
